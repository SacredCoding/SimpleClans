...
public class SCPlayerListener implements Listener {
    ...
    private void PlayerJoinProcess(ClanPlayer cp, Player player)
    {
        if (cp == null)
        {
            return;
    ...
    public void onPlayerJoin(PlayerJoinEvent event)
    {
        ...
        SimpleClans.getInstance().getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {
            ...
            public void run()
            {
                ...
                {
                    SimpleClans.getInstance().getClanManager().reloadClanPlayerAsync(player);
                }
                else
                {
     ...
     */
    @EventHandler
    public void onPlayerJoinReload(PlayerLoadSucessEvent event)
    {
        ClanPlayer cp = SimpleClans.getInstance().getClanManager().getClanPlayer(event.getPlayer());
        if (cp != null)
        {
            PlayerJoinProcess(cp, event.getPlayer());
        }
    }
